...
The [Cross-Consensus Message (XCM)](https://wiki.polkadot.network/docs/learn-crosschain) format defines how messages can be sent between interoperable blockchains. This format opens the door to transfer messages and assets between Moonbeam and Moonriver and the relay chain or other parachains in the Polkadot and Kusama ecosystems, respectively.

Substrate assets are natively interoperable, however, to interact with and transfer these assets it requires tapping into the Substrate API. This can be done via Substrate's assets pallet with knowledge of Rust or through Polkadot.js Apps, which makes the developer experience unideal. 

To improve on the developer experience, XC-20s have been introduced. XC-20s are cross-chain assets on Moonbeam that aim to be one-to-one representations of the origin chain’s native tokens. For example, Kusama's KSM representation will be known as xcKSM. XC-20s allow users to bring their tokens to Moonbeam and interact with them through a familiar [ERC-20 interface](https://github.com/PureStake/moonbeam/blob/master/precompiles/assets-erc20/ERC20.sol) via an XC-20 precompile. Please note that that XC-20 precompiles do not support cross-chain transfers. This is intentionally done to stay as close as possible to the standard ERC-20 interface. 

For more information on XCM, you can check out the [XCM Overview](/builders/xcm/overview/) page of our documentation.
...
The cross-chain asset address is calculated using the following:
...
The hex value that was already calculated is 32 characters long, so prepending 8 `F`s to the hex value will give you the 40 character address you need to interact with the asset precompile. For this example, the full address is `0xFFFFFFFF1FCACBD218EDC0EBA20FC2308C778080`.
...
Now that you've calculated the XC-20 precompile address, you can use the address to interact with the asset like you would with any other ERC-20 in Remix.
