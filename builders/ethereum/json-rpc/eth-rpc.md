...
## Unsupported Ethereum JSON-RPC Methods {: #unsupported-rpc-methods }
 ...
 - **[eth_createAccessList](https://docs.alchemy.com/reference/eth-createaccesslist){target=\_blank}** - creates an EIP2930 type `accessList` based on a given transaction object
 - **[eth_sign](https://support.metamask.io/privacy-and-security/what-is-eth_sign-and-why-is-it-a-risk/){target=\_blank}** - presents a security risk due to signing an arbitrary hash which can be fraudulently applied to other transactions
 - **[eth_signTransaction](https://ethereum.org/en/developers/docs/apis/json-rpc/#eth_signtransaction){target=\_blank}** - presents a security risk due to signing a transaction to be sent at a later time
