...
![SubQuery Banner](/images/builders/integrations/indexers/subsquid/subsquid-banner.png)
...
This guide will explain how to create a Subsquid project that indexes ERC-721 token transfers on the Moonriver network. As such, we will be looking at the `Transfer` EVM event topics.
...
The subsquid template comes with a `package.json` file, defining its dependencies, which we need to install, by running:
...
```bash
npm i @ethersproject/abi ethers
```
...
To finalize this step, it is necessary to run the `codegen` tool, to generate TypeScript Entity classes for our schema definition. You will find the autogenerated files under `src/model/generated`, after running the following command.
...
We suggest creating a dedicated `abi` folder in the project, as we'll be writing additional code to create a programmatic interface around the JSON file. This will also be useful, in case you want to treat multiple types of contracts, with different ABIs. Let's create the folder and an empty file:

```bash
mkdir src/abis
touch src/ERC721.json
```
...
export const INDEXER = 'https://moonriver-beta.indexer.gc.subsquid.io/v4/graphql'
