...
As covered in the [XC-20 Overview](/builders/interoperability/xcm/xc20/overview){target=_blank}, there are two [types of XC-20s](/builders/interoperability/xcm/xc20/overview#types-of-xc-20s){target=_blank}: external and [local](/builders/interoperability/xcm/xc20/xcm-erc20s){target=_blank}. The key distinction between external and local XC-20s, is that local XC-20s are EVM ERC-20s, while external XC-20s are Substrate assets with an ERC-20 interface on top.

Although XC-20s are Substrate assets, Moonbeam has abstracted away low-level interactions with the Substrate API via an easy-to-use [ERC-20 interface](#the-erc20-interface) via a precompile contract. This allows developers to interact with external XC-20s as they would with any other ERC-20. Please note that XC-20 Precompiles do not support cross-chain transfers, and this is intentionally done to stay as close as possible to the standard ERC-20 interface.

External XC-20 assets will all have _xc_ prepended to their names. For example, Kusama's KSM representation on Moonriver will be known as _xcKSM_. They will need to be registered and linked to another asset in the ecosystem before being used. This is done through a whitelisting process via a democracy proposal. 
...
This guide will cover the available external XC-20s, how to calculate precompile addresses for external XC-20s, and how to interact with them via the ERC-20 Solidity interface.
...
With the results from the metadata, you can see that the asset ID corresponds to the xcUNIT external XC-20.
...
This section of the guide will show you how to interact with external XC-20s via the ERC-20 interface using [Remix](/builders/build/eth-api/dev-env/remix){target=_blank}. 

In order to interact with the ERC-20 interface, you'll need to first calculate the precompile address of the XC-20 asset you want to interact with. Then, you can interact with the ERC-20 interface as you would with any other ERC-20. 

You can adapt the instructions in this section for the [Permit.sol](https://github.com/PureStake/moonbeam/blob/master/precompiles/assets-erc20/Permit.sol){target=_blank} interface.
