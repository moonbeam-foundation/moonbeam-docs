...
An orbiter may leave the program and receive their bond back without any delay. The only limitation on this is if the orbiter is currently active they cannot leave. Once they are no longer active they can leave at any time by following these steps: 
...
2. Execute `moonbeamOrbiters.orbiterUnregister(collatorsPoolCount)` where `collatorsPoolCount` is the amount of pools (you can count on the previous chain state call. On Moonbeam it should be 5 as of writing this).
