...
description: How to vote a proposal so that it is enacted or rejected on Moonbeam via governance features
---
...
With the release of [Moonbase Alpha v6](https://github.com/PureStake/moonbeam/releases/tag/v0.6.0), users of the network can now submit proposals for public referenda and vote on them. This guide outlines how to create vote on a proposal that has reached a public referendum. You can find a guide on how to submit a proposal [here](/governance/proposals/).
...
This section goes over the process of voting on a referendum. The guide assumes there is one taking place already, in this case, the one created in [this guide](/governance/proposals/).
...
Voting on Moonbeam is pretty straightforward. Everything related to governance lives under the "Democracy" tab, where (in the image) you can note there is a `1`, indicating there is one referendum taking place. Once there, click on the "Vote" button.
...
Here, you need to provide the following information:
 ...
 2. Enter the number of tokens you want to vote with. These will be locked for the amount of time specified in the next step
 3. Set the vote conviction, which determines its weight (`vote_weight = tokens * conviction_multiplier`). The conviction multiplier is related to the number of enactment periods the tokens will be locked. Consequently, the longer you are willing to lock your tokens, the stronger your vote will be weighted. You also have the option of not locking tokens at all, but vote weight is drastically reduced
...
From the previous image, there are some key takeaways:
 ...
 - Alice's weighted vote is 10800 units. That is, her 1800 tokens locked multiplied by her conviction of x6
 - Charley's weighted vote is 80 units. That is, his 800 tokens with no locking period, meaning his conviction factor was x0.1
 - Both the remaining voting period and time before the proposal is enacted (if passed) are shown on the screen
 - The overall turnout (in percent) is just 0.21%. This is calculated as the total number of voting tokens (2600) divided by the total amount of tokens in the network (1.22M in this case)
...
Public referenda use a positive turnout bias metric, that is, a super-majority approve formula. The equation is the following: 
