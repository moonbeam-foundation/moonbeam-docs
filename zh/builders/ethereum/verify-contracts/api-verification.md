...
验证智能合约可以大大提高其透明度和安全性。部署在 Moonbeam 网络上的智能合约可以通过基于 API 的工具进行验证，包括 Moonscan API 和 Sourcify。

本页面将概述使用这些基于 API 的工具来验证智能合约，或检索 Moonbeam 网络上智能合约的验证状态和元数据的步骤。
...
[Etherscan API](https://docs.etherscan.io){target=\_blank} 提供了各种端点，用于验证智能合约、检索已验证的合约 ABI 和源代码，以及与 Moonbeam 网络上的已验证合约进行交互。

### 生成 Etherscan API 密钥 {: #generating-an-etherscan-api-key }

在使用 Moonscan API 之前，您需要生成一个 Etherscan API 密钥。请按照 Etherscan 插件验证页面的[密钥生成部分](/builders/ethereum/verify-contracts/etherscan-plugins/#generating-an-etherscan-api-key){target=_blank}中的说明进行操作，因为生成的 API 密钥可用于两者。

### Moonscan公共API URL {: #moonscan-public-api-url }

Moonbeam网络的Moonscan API URL如下：
...
要使用 Moonscan API 验证已部署合约的源代码，您必须创建一个包含所有相关合约创建信息的 POST 请求，并将该请求发送到 Moonscan 的 REST API。以下是使用 JavaScript 和 [Axios](https://axios-http.com/docs/intro){target=_blank}（一个 HTTP 客户端）的示例代码：
...
===
    ...
    const response = await axios.post(
      'https://api-moonbeam.moonscan.io/api',
      {
        ...
        contractname: 'INSERT_CONTRACT_NAME', // 如果 codeformat = solidity-standard-json-input，则输入 contractname，例如：erc20.sol:erc20
        compilerversion: 'INSERT_COMPILER_VERSION', // 请参阅 https://etherscan.io/solcversions 以获取支持的版本列表
        ...
        licenseType: 1, // 有效代码 1-14，其中 1=无许可证 ... 14=商业来源许可证 1.1，请参阅 https://etherscan.io/contract-license-types
        libraryname1: 'INSERT_LIBRARY_NAME', // 如果适用，请输入使用的第一个库的名称，即 SafeMath（最多可以使用 10 个库）
        libraryaddress1: 'INSERT_LIBRARY_ADDRESS', // 如果适用，请输入使用的第一个库的地址
        libraryname2: 'INSERT_LIBRARY_NAME', // 如果适用，请输入使用的第二个库的名称
        libraryaddress2: 'INSERT_LIBRARY_ADDRESS', // 如果适用，请输入使用的第二个库的地址
        // ...
      },
...
Sourcify使用链ID来标识请求的目标网络。Moonbeam网络的链ID如下：
...
部分匹配指的是链上合约的已部署字节码与使用元数据和源文件（除了元数据哈希）重新编译产生的字节码匹配的情况。对于部分匹配，已部署的合约和给定的源代码及元数据在功能上是相同的，但源代码注释、变量名或其他元数据字段（如源路径）存在差异。
...
或者，您也可以使用 [Sourcify 托管的 GUI](https://sourcify.dev/#/verifier){target=_blank} 提交合约以进行验证。
...
Foundry 的 Forge 工具内置了对 Sourcify 验证的支持，类似于它[内置的对 Etherscan 的支持](/builders/ethereum/verify-contracts/etherscan-plugins/#using-foundry-to-verify){target=_blank}。本指南的这一部分中的示例将使用在[使用 Foundry 部署到 Moonbeam](/builders/ethereum/dev-env/foundry/){target=_blank} 指南中创建的 `MyToken.sol` 合约。
