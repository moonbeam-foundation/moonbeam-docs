...
The end-user can retrieve price feeds with read-only operations via a Consumer contract, referencing the correct Aggregator interface (Proxy contract). The Proxy acts as a middle-ware to provide the Consumer with the most up-to-date Aggregator for a particular price feed. 
...
Let' go ahead and use the interface contract to fetch the price feed of `BTC to USD` using [Remix](/integrations/remix/). 
