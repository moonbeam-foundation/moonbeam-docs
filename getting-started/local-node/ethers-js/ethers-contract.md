...
The guide assumes that you have a local Moonbeam node running in `--dev` mode. You can find instructions on how to set up a local Moonbeam node [here](/getting-started/local-node/setting-up-a-node/).
...
As of writing this guide, the versions used were 15.2.1 and 7.0.8, respectively.
...
With the package.json file created, we can then install both the ethers.js and the Solidity compiler packages (fixed at version v0.7.4) by executing:
...
To verify the installed version of ethers.js or the Solidity compiler, use the `ls` command:
...
As of writing this guide, the versions used were 5.0.22 and 0.7.4 (as mentioned before), respectively.

For this example, we will use a similar setup to our [web3.js contract tutorial](/getting-started/local-node/web3-js/web3-contract/) so some files will look similar:

-  _Incrementer.sol_: the file with our Solidity code
-  _compile.js_: will compile the contract with the Solidity compiler
-  _deploy.js_: will handle the deployment to our local Moonbeam node
-  _get.js_: will make a call to the node to get the current value of the number
-  _increment.js_: will make a transaction to increment the number stored on the Moonbeam node
...
The only purpose of the _compile.js_ file (arbitrarily named and can be found [here](/code-snippets/web3-contract-local/compile.js)) is to use the Solidity compiler to output the bytecode and interface of our contract.
...
And finally, we run the compiler and extract the data related to our incrementer contract because that is all we need for this simple example.
...
!!! note
    The _deploy.js_ script provides the contract address as an output. This comes in handy because, as it is used for the contract interaction files.
...
First, let's review the _get.js_ file (the simplest of them all, which can be found [here](/code-snippets/ethers-contract-local/get.js)), which fetches the current value stored on the contract. We need to load our ethers.js module and the export of the _compile.js_ file, from which we will extract the `abi`. Next, we define the provider so we can access the methods necessary to call our contract
...
With all the files ready, we can proceed to deploy our contract to the local Moonbeam node. To do this, we execute the following command in the directory where all the files are:
...
This example provides context on how to start working with Moonbeam and trying out its Ethereum compatibility features, such as the ethers.js library. We are interested in hearing about your experience following the steps in this guide or trying other Ethereum-based tools with Moonbeam. Feel free to join us in the [Moonbeam Discord here](https://discord.gg/PfpUATX). We would love to hear your feedback and answer any questions that you have.
