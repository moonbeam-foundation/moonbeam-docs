...
const main = async () => {
  ...
  try {
    ...
    const tx = api.tx.parachainStaking.scheduleCandidateBondLess(
      decreaseAmount
    );
    ...
    await tx.signAndSend(candidate, ({ status, events }) => {
      if (status.isInBlock) {
        ...
        events.forEach(({ event }) => {
          ...
          if (section === 'parachainStaking' && method === 'CandidateBondLessRequested') {
        ...
        api.query.parachainStaking.candidateInfo(candidate.address).then(finalState => {
